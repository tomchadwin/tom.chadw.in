...
describe("Duck texture", function() {
	it("Renders tinted in a cube", function() {
...
void main() {
	...
	gl_Position = uProjMatrix * uModelViewMatrix * vec4(aPos, 1.);
			...
			varyings: { vTexCoords: "vec2", vTint: "vec3" },
...
void main() {
	...
	gl_FragColor *= vec4(vTint, 1.0);
		...
		return imagePromiseFromUrl("./testimages.org/ducks.png").then((ducksImage) => {
			...
			function renderAtTime(sec) {
				...
				return expectPixelmatch(
					context,
					`spec/texture/duck-cube/duck-cube-${sec}`,
					190
				);
			}
